...
const Header = ({ theme, setTheme }) => {
  ...
  useEffect(() => {
    if (searchQuery.length >= 1) {
      setLoading(true);
      axios.get(`https://unitywomen-48288fd0e24a.herokuapp.com/api/qolbaq?search=${searchQuery}`)
        .then(response => {
              ...
              <ul className="search-results absolute top-full left-0 w-full bg-white dark:bg-black shadow-md rounded-lg max-h-80 overflow-y-auto mt-1 z-10">
                {loading ? (
                  ...
                  filteredItems.map((item, index) => (
                    ...
                    >
                      <img src={item.photo} alt={item.title} className="w-12 h-12 object-cover rounded" />
