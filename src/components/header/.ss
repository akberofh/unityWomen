          ...
          {loading ? (
            ...
            <ul className="search-results absolute top-full left-0 w-full bg-white dark:bg-black shadow-md rounded-lg max-h-80 overflow-y-auto mt-1">
              {Array.isArray(items) && items.length > 0 ? (
                ...
                <li className="p-2 text-center text-gray-500 dark:text-gray-300">Hiçbir ürün bulunamadı.</li>
              )}
            </ul>
          )}
        </div>
      ...
      {isMenuOpen && (
        <div className="md:hidden bg-white dark:bg-black py-4 px-6 space-y-4">
          <div className="relative group">
            ...
            {isCategoryMenuOpen && (
              <div className="absolute left-0 bg-white dark:bg-black text-gray-700 dark:text-white shadow-md rounded-lg mt-2 w-full">
                {categories.map((category) => (
                  <Link
                    key={category.name}
                    to={category.link}
